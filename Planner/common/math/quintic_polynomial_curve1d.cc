...
QuinticPolynomialCurve1d::QuinticPolynomialCurve1d(const double x0, const double dx0,
...
double QuinticPolynomialCurve1d::Evaluate(const uint32_t order, const double p) const {
    switch (order) {
        case 0: {
            return ((((coef_[5] * p + coef_[4]) * p + coef_[3]) * p + coef_[2]) * p + coef_[1]) *
                       p +
                   coef_[0];
        }
        case 1: {
...
std::string QuinticPolynomialCurve1d::ToString() const { return {}; }

double QuinticPolynomialCurve1d::Coef(const size_t order) const { return coef_[order]; }
}    // namespace math
}    // namespace common
