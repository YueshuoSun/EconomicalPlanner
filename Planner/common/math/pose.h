...
class Pose {
   public:
    ...
    inline Pose relativeTo(const Pose &coord) const {
        ...
        return {dx * std::cos(coord.theta()) + dy * std::sin(coord.theta()),
                -dx * std::sin(coord.theta()) + dy * std::cos(coord.theta()),
                theta_ - coord.theta()};
    }
    ...
    inline Pose transform(const Pose &relative) const {
        return {x_ + relative.x() * std::cos(theta_) - relative.y() * std::sin(theta_),
                y_ + relative.x() * std::sin(theta_) + relative.y() * std::cos(theta_),
