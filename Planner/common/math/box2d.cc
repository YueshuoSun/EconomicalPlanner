...
void Box2d::Extend(double top, double right, double bottom, double left) {
    length_ += left + right;
    width_ += top + bottom;
    half_length_ = length_ / 2.0;
    half_width_ = width_ / 2.0;

    double dx = (right - left) * cos_heading_ - (top - bottom) * sin_heading_;
    double dy = (right - left) * sin_heading_ + (top - bottom) * cos_heading_;
    center_ += Vec2d(dx / 2, dy / 2);
    InitCorners();
}
...
std::string Box2d::DebugString() const {
    return ::common::StrCat("box2d ( center = ", center_.DebugString(), "  heading = ", heading_,
