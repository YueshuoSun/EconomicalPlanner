...
void DiscretizedTrajectory::ResampleTrajectory(double resample_step) {
    resample_step_ = 0.0;
    int len = round((data_.back().s - data_.front().s) / resample_step) + 1;
    std::vector<double> s_vec = common::util::LinSpaced(data_.front().s, data_.back().s, len);
    DataType data;
    for (int i = 0; i < len; i++) {
    ...
    // data.push_back(EvaluateStation(data_.back().s));
    data_ = std::move(data);
    resample_step_ = resample_step;
}
DiscretizedTrajectory::DiscretizedTrajectory(const DiscretizedTrajectory &rhs, size_t begin,
                                             size_t end) {
    if (end < 0) {
        end = rhs.data_.size();
    }
...
DiscretizedTrajectory::DataType::const_iterator DiscretizedTrajectory::QueryLowerBoundStationPoint(
    ...
    return std::lower_bound(data_.begin(), data_.end(), station,
                            [](const TrajectoryPoint &t, double station) { return t.s < station; });
}
TrajectoryPoint LinearInterpolateTrajectory(const TrajectoryPoint &p0, const TrajectoryPoint &p1,
                                            double s) {
    double s0 = p0.s;
    double s1 = p1.s;
...
DiscretizedTrajectory::DataType::const_iterator DiscretizedTrajectory::QueryLowerBoundTimePoint(
    double timestamp) const {
    if (timestamp >= data_.back().t) {
        return data_.end() - 1;
    }

    return std::lower_bound(data_.begin(), data_.end(), timestamp,
                            [](const TrajectoryPoint &t, double time) { return t.t < time; });
}

DiscretizedTrajectory::DataType::const_iterator DiscretizedTrajectory::QueryNearestPoint(
...
Vec2d DiscretizedTrajectory::GetProjection(const Vec2d &xy) const {
    ...
    if (index_start < index_end) {
        ...
        project_point = LinearInterpolateTrajectory(data_[index_start], data_[index_end],
                                                    data_[index_start].s + delta_s);
    }

    double nr_x = xy.x() - project_point.x, nr_y = xy.y() - project_point.y;
    double lateral = copysign(hypot(nr_x, nr_y),
                              nr_y * cos(project_point.theta) - nr_x * sin(project_point.theta));
