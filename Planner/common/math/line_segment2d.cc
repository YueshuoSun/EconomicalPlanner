...
std::vector<Vec2d> LineSegment2d::ObsSamplePoints(double step) {
    int num = static_cast<int>(round(length_ / step - 1));
    std::vector<Vec2d> points(num);

    for (int i = 0; i < num; ++i) {
        points[i] = start_ + unit_direction_ * step * (i + 1);
    }
