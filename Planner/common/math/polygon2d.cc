...
double Polygon2d::DistanceTo(const Box2d &box) const { return DistanceTo(Polygon2d(box)); }
...
std::vector<std::array<double, 3>> Polygon2d::GetConvexSet() const {
    if (!is_convex_) {
        return {};
    }

    std::vector<std::array<double, 3>> constraints(line_segments_.size());
    for (size_t i = 0; i < line_segments_.size(); i++) {
        auto &line = line_segments_[i];
        double direc = CrossProd(line.start(), line.end(), center_) < kMathEpsilon ? 1 : -1;
        // Ax + By + C < 0
        constraints[i][0] = direc * (line.start().y() - line.end().y());
        constraints[i][1] = direc * (line.end().x() - line.start().x());
        constraints[i][2] = direc * (line.start().CrossProd(line.end()));
    }
    return constraints;
}
...
std::vector<std::vector<double>> Polygon2d::GetLineSegParams() {
    std::vector<double> ABC(3, 0.0);
    std::vector<std::vector<double>> res;
    auto p_c = this->center();
    for (auto line : line_segments_) {
        ABC[0] = line.end().y() - line.start().y();    // verify
        ABC[1] = line.start().x() - line.end().x();
        ABC[2] = line.start().y() * line.end().x() - line.end().y() * line.start().x();
        if (ABC[0] * p_c.x() + ABC[1] * p_c.y() + ABC[2] > 0) {
            ABC[0] = -ABC[0];
            ABC[1] = -ABC[1];
            ABC[2] = -ABC[2];
        }
