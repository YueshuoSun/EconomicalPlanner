...
struct TrajectoryPoint {
    ...
    explicit TrajectoryPoint(const common::math::Pose &pose)
        : x(pose.x()), y(pose.y()), theta(pose.theta()) {}

    explicit operator common::math::Pose() const { return {x, y, theta}; }
};

inline TrajectoryPoint LinearInterpolateStation(const TrajectoryPoint &p0,
                                                const TrajectoryPoint &p1, double s0, double s1,
                                                double s) {
...
class DiscretizedTrajectory {
   public:
    ...
    explicit DiscretizedTrajectory(std::vector<TrajectoryPoint> points, double resolution)
    ...
    inline void delete_data(size_t begin, size_t end = -1) {
    ...
    DataType::const_iterator QueryNearestPoint(const Vec2d &point,
